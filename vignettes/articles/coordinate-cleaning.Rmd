...
```{r download-native}
native_range <- get_distribution("Callitris rhomboidea", rank="species",
...
```{r plot-native}
(p <- plot_distribution(native_range, crop.map=TRUE) + 
...
An appropriate buffer width might be wider or narrower, depending on downstream analyses but visualisation can help to sanity-check the buffer before discarding points. 
