...
checklist <- generate_checklist("Acacia", rank="genus", area="CPP",
...
These arguments can be combined in the `generate_checklist`, `generate_occurrence_matrix`, and `summary_table` to generate outputs for focal taxa in a desired area. For example, Myrtaceae species in Brazil:

```{r filter-example}
checklist <- generate_checklist("Myrtaceae", rank="family", 
                                area=c("BZC", "BZN", "BZS", "BZE", "BZL"))
```

### A note on filtering by taxon

When filtering by taxon, you need to tell the function what taxonomic rank the name you're providing is using the `rank` argument.

For example, making a summary table for the genus *Poa*:
```{r filter-genus}
summary_table("Poa", rank="genus")
```
...
summary_table("Poales", rank="order")
```
...
```{r filter-africa}
summary_table("Poa", rank="genus", area=get_wgsrpd3_codes("Southern Hemisphere"))
```
