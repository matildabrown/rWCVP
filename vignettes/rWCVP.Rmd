...
checklist <- wcvp_checklist("Acacia", taxon.rank="genus", area="CPP",
...
```{r filter-example}
checklist <- wcvp_checklist("Myrtaceae", taxon.rank="family", 
...
```{r filter-genus}
wcvp_summary("Poa", taxon.rank="genus")
```
...
wcvp_summary("Poales", taxon.rank="order")
```
...
```{r filter-africa}
wcvp_summary("Poa", taxon.rank="genus", area=get_wgsrpd3_codes("Southern Hemisphere"))
```
