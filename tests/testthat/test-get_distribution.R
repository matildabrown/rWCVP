...
test_that("correct geography for example", {
  ...
  distribution <- wcvp_distribution(example, example_rank, introduced=FALSE,
...
test_that("returns a spatial dataframe", {
  distribution <- wcvp_distribution("Myrcia guianensis", "species")
  expect_s3_class(distribution, "sf")
})

test_that("returns expected distribution for Poa annua", {
  taxon <- "Poa annua"
  distribution <- wcvp_distribution(taxon, "species")
