...
```{r, echo = FALSE, results = "asis"}
  cat("\n")
if(input.area != "global")  cat("### Geography: ", paste(get_area_name(input.area), collapse = " + "),"\n") else cat("### Geography: Global","\n")
```
...
df %>% 
  ...
  {if(input.area != "global") dplyr::filter(.,area_code_l3 %in% input.area) else unique(.)} %>% 
  unique() %>% 
  na.omit() %>% 
...
df3[is.na(df3$accepted_name),"rowgrouptext"] <- "Unplaced"
  ...
  dplyr::select(maintext, plant_name_id, rowgrouptext) %>% 
