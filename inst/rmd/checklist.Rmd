...
```{r, echo = FALSE, results = "asis"}
if(!is.null(taxa))  taxa = "All taxa"
  cat("### Taxa: ", paste(taxa, collapse = " + "),  "\n")
  cat("\n")
if(synonyms==TRUE) {
  cat("**Including synonyms, illegitimate, invalid and other non-accepted names.**") 
} else {cat("**Accepted and unplaced names only.** ")}
 
```

```{r, echo = FALSE, results = "asis"}
  cat("\n")
  cat("### Geography: ", paste(get_area_name(input.area), collapse = " + "),"\n")
```
...
--------------   ----------------
E                Endemic to a single WGSPRD Level 3 area
e                Regionally endemic (to the set of areas shown in **Geography** above)
x                Considered extinct or eradicated
i                Introduced and naturalised
?                Occurrence doubtful
 --------------   ----------------
...
n.sp= c(nrow(unique(df %>% 
     dplyr::filter(taxon_status %in% c("Accepted","Unplaced"),
            taxon_rank == "Species") %>% 
...
df3 %>% dplyr::filter(taxon_rank!= "Genus") %>% 
  dplyr::select(family, endemic_flag, maintext, plant_name_id) %>% 
